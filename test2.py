...
def horizontal(i):
	...
	awal_w = 0
	for x in m:
		var_kanan  = "".join(x).lower() 
		var_kiri   = "".join(x[::-1]).lower()
		index_cell = m.index(list(var_kanan)) 

		for x in range(len(var_kanan)):
			if (W[0]==var_kanan[x]): 
				fillt_kanan = var_kanan[x:] 
				split_wordR = fillt_kanan[awal_w:len_w] 
